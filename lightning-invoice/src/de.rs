...
impl FromStr for SignedRawBolt11Invoice {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		Ok(SignedRawBolt11Invoice {
			raw_invoice: RawBolt11Invoice { hrp: raw_hrp, data: data_part },
...
mod test {
	...
	fn from_bech32(bytes_5b: &[u8]) -> Vec<u5> {
		bytes_5b.iter().map(|c| u5::try_from_u8(CHARSET_REV[*c as usize] as u8).unwrap()).collect()
	}
	...
	fn test_parse_fallback() {
		...
		let cases = vec![
			...
			(vec![u5::try_from_u8(21).unwrap(); 41], Err(Bolt11ParseError::Skip)),
			...
			(vec![u5::try_from_u8(17).unwrap(); 1], Err(Bolt11ParseError::InvalidPubKeyHashLength)),
			(vec![u5::try_from_u8(18).unwrap(); 1], Err(Bolt11ParseError::InvalidScriptHashLength)),
		];
