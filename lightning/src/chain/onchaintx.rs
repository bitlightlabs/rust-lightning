...
use crate::sign::{
	ecdsa::EcdsaChannelSigner, ChannelDerivationParameters, ChannelSigner, EntropySource,
...
impl<'a, 'b, ES: EntropySource, SP: SignerProvider>
	ReadableArgs<(&'a ES, &'b SP, u64, [u8; 32], PathBuf)> for OnchainTxHandler<SP::EcdsaSigner>
{
	fn read<R: io::Read>(
		reader: &mut R, args: (&'a ES, &'b SP, u64, [u8; 32], PathBuf),
		...
		let mut pending_claim_requests = hash_map_with_capacity(cmp::min(
		...
		let mut claimable_outpoints = hash_map_with_capacity(cmp::min(
			pending_claim_requests_len as usize,
			MAX_ALLOC_SIZE / 128,
		));
...
impl<ChannelSigner: EcdsaChannelSigner> OnchainTxHandler<ChannelSigner> {
				...
				match claim {
					OnchainClaim::Tx(tx) => {
						if tx.is_fully_signed() {
							...
							log_info!(
								logger,
								"Waiting for signature of unsigned onchain transaction {}",
								tx.0.compute_txid()
							);
						}
					},
		...
		F::Target: FeeEstimator,
	{
		...
		if cached_request.is_malleable() {
			...
			if let Some((output_value, new_feerate)) = cached_request.compute_package_output(
				predicted_weight,
				self.destination_script.minimal_non_dust().to_sat(),
				feerate_strategy,
				conf_target,
				fee_estimator,
				logger,
		...
		F::Target: FeeEstimator,
	{
		...
		for mut req in preprocessed_requests {
			if let Some((new_timer, new_feerate, claim)) = self.generate_claim(
				...
				debug_assert!(
					cfg!(fuzzing) || self.pending_claim_requests.get(&claim_id).is_none()
				);
				...
				match bump_claim {
					OnchainClaim::Tx(bump_tx) => {
						if bump_tx.is_fully_signed() {
							log_info!(
								logger,
								"Broadcasting RBF-bumped onchain {}",
								log_tx!(bump_tx.0)
							);
	...
	pub(super) fn transaction_unconfirmed<B: Deref, F: Deref, L: Logger>(
		&mut self, txid: &Txid, broadcaster: B, conf_target: ConfirmationTarget,
		fee_estimator: &LowerBoundedFeeEstimator<F>, logger: &L,
	) where
		B::Target: BroadcasterInterface,
		F::Target: FeeEstimator,
	...
	pub(crate) fn get_maybe_signed_htlc_tx(
		...
		let get_signed_htlc_tx = |holder_commitment: &HolderCommitmentTransaction| {
			...
			if let Ok(htlc_sig) = self.signer.sign_holder_htlc_transaction(
				&htlc_tx,
				0,
				&htlc_descriptor,
				&self.secp_ctx,
