...
impl Readable for ChannelMonitorUpdate {
	fn read<R: io::Read>(r: &mut R) -> Result<Self, DecodeError> {
		...
		let mut updates = Vec::with_capacity(cmp::min(
			len as usize,
			MAX_ALLOC_SIZE / ::core::mem::size_of::<ChannelMonitorUpdateStep>(),
		));
		for _ in 0..len {
...
impl_writeable_tlv_based!(HTLCUpdate, {
	...
	(2, source, required),
	(4, payment_preimage, option),(6, htlc_value_rgb, option),
	...
	fn confirmation_threshold(&self) -> u32 {
		...
		match self.event {
			OnchainEvent::MaturingOutput {
				...
				conf_threshold =
					cmp::max(conf_threshold, self.height + descriptor.to_self_delay as u32 - 1);
			},
...
impl_writeable_tlv_based_enum_upgradable!(OnchainEvent,
	(0, HTLCUpdate) => {
		...
		(3, commitment_tx_output_idx, option),(4, htlc_value_rgb, option),
...
impl Writeable for IrrevocablyResolvedHTLC {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
		let mapped_commitment_tx_output_idx =
			self.commitment_tx_output_idx.unwrap_or(u32::max_value());
		write_tlv_fields!(writer, {
...
impl Readable for IrrevocablyResolvedHTLC {
	fn read<R: io::Read>(reader: &mut R) -> Result<Self, DecodeError> {
		...
		Ok(Self {
			commitment_tx_output_idx: if mapped_commitment_tx_output_idx == u32::max_value() {
				None
			} else {
				Some(mapped_commitment_tx_output_idx)
			},
			resolving_txid,
			payment_preimage,
			resolving_tx,
	...
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), Error> {
		...
		for (ref txid, ref htlc_infos) in self.counterparty_claimable_outpoints.iter() {
			...
			for &(ref htlc_output, ref htlc_source) in htlc_infos.iter() {
				debug_assert!(
					htlc_source.is_none()
						|| Some(**txid) == self.current_counterparty_commitment_txid
	...
	L::Target: Logger,
{
	...
	pub(crate) fn from_impl<S: EcdsaChannelSigner>(
...
impl<Signer: EcdsaChannelSigner> ChannelMonitor<Signer> {
	...
	pub(crate) fn new(
		...
		funding_info: (OutPoint, ScriptBuf), channel_parameters: &ChannelTransactionParameters,
		...
		counterparty_node_id: PublicKey, channel_id: ChannelId, ldk_data_dir: PathBuf,
		...
		let counterparty_channel_parameters =
			channel_parameters.counterparty_parameters.as_ref().unwrap();
		let counterparty_delayed_payment_base_key =
			counterparty_channel_parameters.pubkeys.delayed_payment_basepoint;
		let counterparty_htlc_base_key = counterparty_channel_parameters.pubkeys.htlc_basepoint;
		let counterparty_commitment_params = CounterpartyCommitmentParameters {
		...
		outputs_to_watch.insert(
			funding_info.0.txid,
			vec![(funding_info.0.index as u32, funding_info.1.clone())],
		);
		...
		L::Target: Logger,
	{
		...
		inner.provide_initial_counterparty_commitment_tx(
			txid,
			htlc_outputs,
			commitment_number,
			their_cur_per_commitment_point,
			feerate_per_kw,
	...
	fn provide_latest_counterparty_commitment_tx<L: Deref>(
		&self, txid: Txid, htlc_outputs: Vec<(HTLCOutputInCommitment, Option<Box<HTLCSource>>)>,
		commitment_number: u64, their_per_commitment_point: PublicKey, logger: &L,
	) where
		L::Target: Logger,
	{
		let mut inner = self.inner.lock().unwrap();
	...
	pub(crate) fn provide_payment_preimage<B: Deref, F: Deref, L: Deref>(
		&self, payment_hash: &PaymentHash, payment_preimage: &PaymentPreimage, broadcaster: &B,
		...
		L::Target: Logger,
	{
		...
		for (txid, outputs) in lock.get_outputs_to_watch().iter() {
			for (index, script_pubkey) in outputs.iter() {
				...
				log_trace!(
					logger,
					"Registering outpoint {} with the filter for monitoring spends",
					outpoint
				);
	...
	pub fn process_pending_events<H: Deref>(&self, handler: &H) -> Result<(), ReplayEvent>
	where
	...
	pub fn sign_to_local_justice_tx(
		...
		self.inner.lock().unwrap().sign_to_local_justice_tx(
			justice_tx,
			input_idx,
			value,
			commitment_number,
	...
	pub fn block_connected<B: Deref, F: Deref, L: Deref>(
		&self, header: &Header, txdata: &TransactionData, height: u32, broadcaster: B,
		...
		L::Target: Logger,
	{
		let mut inner = self.inner.lock().unwrap();
		let logger = WithChannelMonitor::from_impl(logger, &*inner, None);
		inner.block_connected(header, txdata, height, broadcaster, fee_estimator, &logger)
	}

	/// Determines if the disconnected block contained any transactions of interest and updates
	/// appropriately.
	pub fn block_disconnected<B: Deref, F: Deref, L: Deref>(
		&self, header: &Header, height: u32, broadcaster: B, fee_estimator: F, logger: &L,
	) where
		B::Target: BroadcasterInterface,
		F::Target: FeeEstimator,
		L::Target: Logger,
	{
		let mut inner = self.inner.lock().unwrap();
		let logger = WithChannelMonitor::from_impl(logger, &*inner, None);
		inner.block_disconnected(header, height, broadcaster, fee_estimator, &logger)
	}
	...
	pub fn transaction_unconfirmed<B: Deref, F: Deref, L: Deref>(
		&self, txid: &Txid, broadcaster: B, fee_estimator: F, logger: &L,
	) where
		B::Target: BroadcasterInterface,
		F::Target: FeeEstimator,
		L::Target: Logger,
	{
		...
		let mut inner = self.inner.lock().unwrap();
		let logger = WithChannelMonitor::from_impl(logger, &*inner, None);
		inner.transaction_unconfirmed(txid, broadcaster, &bounded_fee_estimator, &logger);
	}
		...
		L::Target: Logger,
	{
		...
		let mut inner = self.inner.lock().unwrap();
		let logger = WithChannelMonitor::from_impl(logger, &*inner, None);
		inner.best_block_updated(header, height, broadcaster, &bounded_fee_estimator, &logger)
	}

	/// Returns the set of txids that should be monitored for re-organization out of the chain.
	pub fn get_relevant_txids(&self) -> Vec<(Txid, u32, Option<BlockHash>)> {
		let inner = self.inner.lock().unwrap();
		let mut txids: Vec<(Txid, u32, Option<BlockHash>)> = inner
	...
	pub fn get_spendable_outputs(
		&self, tx: &Transaction, confirmation_height: u32,
	...
	pub fn is_fully_resolved<L: Logger>(&self, logger: &L) -> bool {
		...
		if is_all_funds_claimed {
			if !inner.funding_spend_seen {
				debug_assert!(
					false,
					"We should see funding spend by the time a monitor clears out"
				);
...
impl<Signer: EcdsaChannelSigner> ChannelMonitorImpl<Signer> {
	...
	fn get_htlc_balance(
		&self, htlc: &HTLCOutputInCommitment, source: Option<&HTLCSource>, holder_commitment: bool,
		counterparty_revoked_commitment: bool, confirmed_txid: Option<Txid>,
	) -> Option<Balance> {
		let htlc_commitment_tx_output_idx = if let Some(v) = htlc.transaction_output_index {
			v
		} else {
			return None;
		};
		...
		for event in self.onchain_events_awaiting_threshold_conf.iter() {
			match event.event {
				OnchainEvent::HTLCUpdate {
					commitment_tx_output_idx, htlc_value_satoshis, ..
				...
				OnchainEvent::HTLCSpendConfirmation {
					commitment_tx_output_idx, preimage, ..
				...
				OnchainEvent::MaturingOutput {
					...
					.as_ref()
					.map(|tx| {
						tx.input.iter().enumerate().any(|(input_idx, inp)| {
		...
		let htlc_resolved = self.htlcs_resolved_on_chain.iter().find(|v| {
			if v.commitment_tx_output_idx == Some(htlc_commitment_tx_output_idx) {
				debug_assert!(htlc_spend_txid_opt.is_none());
		...
		debug_assert!(
			holder_timeout_spend_pending.is_some() as u8
				+ htlc_spend_pending.is_some() as u8
				+ htlc_resolved.is_some() as u8
				<= 1
		);

		let htlc_commitment_outpoint =
			BitcoinOutPoint::new(confirmed_txid.unwrap(), htlc_commitment_tx_output_idx);
		let htlc_output_to_spend = if let Some(txid) = htlc_spend_txid_opt {
		...
		let htlc_output_spend_pending =
			self.onchain_tx_handler.is_output_spend_pending(&htlc_output_to_spend);
		...
		if let Some(conf_thresh) = holder_delayed_output_pending {
			...
			if htlc_output_claim_pending.is_some() {
				...
				debug_assert!(
					holder_timeout_spend_pending.is_none(),
...
impl<Signer: EcdsaChannelSigner> ChannelMonitor<Signer> {
	...
	pub fn get_claimable_balances(&self) -> Vec<Balance> {
		...
		let funding_spend_pending =
			us.onchain_events_awaiting_threshold_conf.iter().find_map(|event| {
				if let OnchainEvent::FundingSpendConfirmation {
				...
				for (htlc, source) in $htlc_iter {
					if htlc.transaction_output_index.is_some() {
						if let Some(bal) = us.get_htlc_balance(
							...
							source,
							$holder_commitment,
							$counterparty_revoked_commitment,
							confirmed_txid,
						) {
							res.push(bal);
						}
		...
		if let Some(txid) = confirmed_txid {
			...
			if let Some(counterparty_tx_htlcs) = us.counterparty_claimable_outpoints.get(&txid) {
				...
				if Some(txid) == us.current_counterparty_commitment_txid
					|| Some(txid) == us.prev_counterparty_commitment_txid
				{
					walk_htlcs!(
						false,
						false,
						counterparty_tx_htlcs.iter().map(|(a, b)| (a, b.as_ref().map(|b| &**b)))
					);
				} else {
					walk_htlcs!(
						false,
					...
					for event in us.onchain_events_awaiting_threshold_conf.iter() {
						...
						} = &event.event
						{
							...
							if let Some(confirmed_to_self_idx) =
								confirmed_counterparty_output.map(|(idx, _)| idx)
							{
								if event
									.transaction
									.as_ref()
									.map(|tx| {
										tx.input.iter().any(|inp| {
											inp.previous_output.vout == confirmed_to_self_idx
										})
									})
									.unwrap_or(false)
								{
					...
					{
						let output_spendable = us.onchain_tx_handler.is_output_spend_pending(
							&BitcoinOutPoint::new(txid, confirmed_to_self_idx),
						);
						if output_spendable {
				...
				walk_htlcs!(
					true,
					false,
					us.current_holder_commitment_tx
						.htlc_outputs
						.iter()
						.map(|(a, _, c)| (a, c.as_ref()))
				);
				if let Some(conf_thresh) = pending_commitment_tx_conf_thresh {
					res.push(Balance::ClaimableAwaitingConfirmations {
				...
				if txid == prev_commitment.txid {
					walk_htlcs!(
						true,
						false,
						prev_commitment.htlc_outputs.iter().map(|(a, _, c)| (a, c.as_ref()))
					);
					if let Some(conf_thresh) = pending_commitment_tx_conf_thresh {
						res.push(Balance::ClaimableAwaitingConfirmations {
					...
					for &(ref htlc, ref source_option) in latest_outpoints.iter() {
						if let &Some(ref source) = source_option {
							res.insert(
								...
								(
									htlc.clone(),
									us.counterparty_fulfilled_htlcs
	...
	pub(crate) fn get_pending_or_resolved_outbound_htlcs(
		&self,
	) -> HashMap<HTLCSource, (HTLCOutputInCommitment, Option<PaymentPreimage>)> {
		let us = self.inner.lock().unwrap();
			...
			($holder_commitment: expr, $htlc_iter: expr) => {
				for (htlc, source) in $htlc_iter {
					if us
						...
						.iter()
						.any(|v| v.commitment_tx_output_idx == htlc.transaction_output_index)
					{
						...
						// indicating we have spent this HTLC with a timeout, claiming it back
						// and awaiting confirmations on it.
						let htlc_update_confd =
							us.onchain_events_awaiting_threshold_conf.iter().any(|event| {
								if let OnchainEvent::HTLCUpdate {
								...
								} = event.event
								{
									...
									Some(commitment_tx_output_idx) == htlc.transaction_output_index
										&& us.best_block.height
											>= event.height + ANTI_REORG_DELAY - 1
								} else if let OnchainEvent::HTLCSpendConfirmation {
						...
						let counterparty_resolved_preimage_opt = us
						...
						if !htlc_update_confd || counterparty_resolved_preimage_opt.is_some() {
							res.insert(
								source.clone(),
								(htlc.clone(), counterparty_resolved_preimage_opt),
							);
						}
					}
				}
			...
			|| Some(txid) == us.prev_counterparty_commitment_txid
		{
			walk_htlcs!(
				false,
				us.counterparty_claimable_outpoints.get(&txid).unwrap().iter().filter_map(
					|(a, b)| {
			...
			walk_htlcs!(
				true,
				us.current_holder_commitment_tx.htlc_outputs.iter().filter_map(|(a, _, c)| {
			...
			if txid == prev_commitment.txid {
				walk_htlcs!(
					true,
					prev_commitment.htlc_outputs.iter().filter_map(|(a, _, c)| {
						if let Some(source) = c {
							Some((a, source))
						} else {
							None
						...
						if let &Some(ref source) = source_option {
							...
							let entry = OnchainEventEntry {
								...
								event: OnchainEvent::HTLCUpdate {
									...
									htlc_value_satoshis: Some(htlc.amount_msat / 1000),htlc_value_rgb: htlc.amount_rgb,
...
pub fn deliberately_bogus_accepted_htlc_witness() -> Vec<Vec<u8>> {
	vec![
		...
		Vec::new(),
		Vec::new(),
		deliberately_bogus_accepted_htlc_witness_program().into(),
	]
	.into()
}
...
impl<Signer: EcdsaChannelSigner> ChannelMonitorImpl<Signer> {
	...
	fn provide_secret(&mut self, idx: u64, secret: [u8; 32]) -> Result<(), &'static str> {
		...
		if let Some(txid) = self.prev_counterparty_commitment_txid.take() {
			if self.current_counterparty_commitment_txid.unwrap() != txid {
				let cur_claimables = self
					.counterparty_claimable_outpoints
					.get(&self.current_counterparty_commitment_txid.unwrap())
					.unwrap();
				for (_, ref source_opt) in self.counterparty_claimable_outpoints.get(&txid).unwrap()
				{
					if let Some(source) = source_opt {
						...
						{
							self.counterparty_fulfilled_htlcs
								.remove(&SentHTLCId::from_source(source));
						}
					}
				}
				for &mut (_, ref mut source_opt) in
					self.counterparty_claimable_outpoints.get_mut(&txid).unwrap()
				{
					*source_opt = None;
				}
			} else {
				assert!(
					cfg!(fuzzing),
					"Commitment txids are unique outside of fuzzing, where hashes can collide"
				);
			}
		}
	...
	fn provide_initial_counterparty_commitment_tx<L: Deref>(
		...
		L::Target: Logger,
	{
		self.initial_counterparty_commitment_info = Some((
			their_per_commitment_point.clone(),
			feerate_per_kw,
		...
		match self.their_cur_per_commitment_points {
			Some(old_points) => {
				if old_points.0 == commitment_number + 1 {
					self.their_cur_per_commitment_points =
						Some((old_points.0, old_points.1, Some(their_per_commitment_point)));
				} else if old_points.0 == commitment_number + 2 {
					if let Some(old_second_point) = old_points.2 {
						self.their_cur_per_commitment_points = Some((
							old_points.0 - 1,
							old_second_point,
							Some(their_per_commitment_point),
						));
					} else {
						self.their_cur_per_commitment_points =
							Some((commitment_number, their_per_commitment_point, None));
					}
				} else {
					self.their_cur_per_commitment_points =
						Some((commitment_number, their_per_commitment_point, None));
	...
	fn provide_latest_holder_commitment_tx(
		...
		mut htlc_outputs: Vec<(HTLCOutputInCommitment, Option<Signature>, Option<HTLCSource>)>,
		claimed_htlcs: &[(SentHTLCId, PaymentPreimage)], nondust_htlc_sources: Vec<HTLCSource>,
	) -> Result<(), &'static str> {
		if htlc_outputs.iter().any(|(_, s, _)| s.is_some()) {
			...
			debug_assert_eq!(
				htlc_outputs.iter().filter(|(_, s, _)| s.is_some()).count(),
				holder_commitment_tx.trust().htlcs().len()
			);
			for (a, b) in htlc_outputs
				.iter()
				.filter(|(_, s, _)| s.is_some())
				.map(|(h, _, _)| h)
				.zip(holder_commitment_tx.trust().htlcs().iter())
			{
				debug_assert_eq!(a, b);
			}
			debug_assert_eq!(
				htlc_outputs.iter().filter(|(_, s, _)| s.is_some()).count(),
				holder_commitment_tx.counterparty_htlc_sigs.len()
			);
			for (a, b) in htlc_outputs
				.iter()
			...
			debug_assert!(htlc_outputs
				.iter()
				.all(|(htlc, _, _)| htlc.transaction_output_index.is_none()));
			...
			{
				if htlc.offered {
					let source = sources_iter
						.next()
						.expect("Non-dust HTLC sources didn't match commitment tx");
		...
		if self.holder_tx_signed {
			return Err(
				"Latest holder commitment signed has already been signed, update is rejected",
			);
		}
		Ok(())
	}
		...
		L::Target: Logger,
	{
		...
		macro_rules! claim_htlcs {
			($commitment_number: expr, $txid: expr, $htlcs: expr) => {
				let (htlc_claim_reqs, _) = self.get_counterparty_output_claim_info(
					$commitment_number,
					$txid,
					None,
					$htlcs,
		...
		if let Some(txid) = self.current_counterparty_commitment_txid {
			if txid == confirmed_spend_txid {
				if let Some(commitment_number) =
					self.counterparty_commitment_txn_on_chain.get(&txid)
				{
					claim_htlcs!(
						*commitment_number,
						txid,
						self.counterparty_claimable_outpoints.get(&txid)
					);
				} else {
					debug_assert!(false);
		...
		if self.broadcasted_holder_revokable_script.is_some() {
			let holder_commitment_tx = if self.current_holder_commitment_tx.txid
				== confirmed_spend_txid
			{
			...
			if let Some(holder_commitment_tx) = holder_commitment_tx {
				...
				// transactions.
				let (claim_reqs, _) = self
					.get_broadcasted_holder_claims(&holder_commitment_tx, self.best_block.height);
				let conf_target = self.closure_conf_target();
				self.onchain_tx_handler.update_claims_view_from_requests(
					claim_reqs,
					self.best_block.height,
	...
	fn generate_claimable_outpoints_and_watch_outputs(
		...
		let commitment_package = PackageTemplate::build_package(
			self.funding_info.0.txid.clone(),
			self.funding_info.0.index as u32,
		...
		if !self.onchain_tx_handler.channel_type_features().supports_anchors_zero_fee_htlc_tx() {
			...
			let unsigned_commitment_tx =
				self.onchain_tx_handler.get_unsigned_holder_commitment_tx();
		...
		L::Target: Logger,
	{
		let (claimable_outpoints, _) =
			self.generate_claimable_outpoints_and_watch_outputs(ClosureReason::HolderForceClosed {
	...
	fn update_monitor<B: Deref, F: Deref, L: Deref>(
		&mut self, updates: &ChannelMonitorUpdate, broadcaster: &B, fee_estimator: &F,
		logger: &WithChannelMonitor<L>,
	) -> Result<(), ()>
	where
		B::Target: BroadcasterInterface,
		...
		L::Target: Logger,
	{
		if self.latest_update_id == CLOSED_CHANNEL_UPDATE_ID
			&& updates.update_id == CLOSED_CHANNEL_UPDATE_ID
		{
			log_info!(
				logger,
				"Applying post-force-closed update to monitor {} with {} change(s).",
				log_funding_info!(self),
				updates.updates.len()
			);
		} else if updates.update_id == CLOSED_CHANNEL_UPDATE_ID {
			log_info!(
				logger,
				"Applying force close update to monitor {} with {} change(s).",
				log_funding_info!(self),
				updates.updates.len()
			);
		} else {
		...
		if updates.update_id == CLOSED_CHANNEL_UPDATE_ID {
			...
			match updates.updates[0] {
				...
				_ => {
					log_error!(
						logger,
						"Attempted to apply post-force-close ChannelMonitorUpdate of type {}",
						updates.updates[0].variant_name()
					);
		...
		for update in updates.updates.iter() {
			match update {
				ChannelMonitorUpdateStep::LatestHolderCommitmentTXInfo {
					...
					log_trace!(
						logger,
						"Updating ChannelMonitor with latest holder commitment transaction info"
					);
					if self.lockdown_from_offchain {
						panic!();
					}
					if let Err(e) = self.provide_latest_holder_commitment_tx(
						...
						log_error!(
							logger,
							"Providing latest holder commitment transaction failed/was refused:"
						);
						log_error!(logger, "    {}", e);
						ret = Err(());
					}
				...
				ChannelMonitorUpdateStep::CommitmentSecret { idx, secret } => {
					...
					if let Err(e) = self.provide_secret(*idx, *secret) {
						...
						log_error!(
							logger,
							"Providing latest counterparty commitment secret failed/was refused:"
						);
						log_error!(logger, "    {}", e);
						ret = Err(());
					}
				...
				ChannelMonitorUpdateStep::ChannelForceClosed { should_broadcast } => {
					log_trace!(
						logger,
						"Updating ChannelMonitor: channel force closed, should broadcast: {}",
						should_broadcast
					);
					...
					if *should_broadcast {
						...
						log_error!(
							logger,
							"    in channel monitor for channel {}!",
							&self.channel_id()
						);
				...
				ChannelMonitorUpdateStep::ShutdownScript { scriptpubkey } => {
					...
					if let Some(shutdown_script) =
						self.shutdown_script.replace(scriptpubkey.clone())
					{
						panic!(
							"Attempted to replace shutdown script {} with {}",
							shutdown_script, scriptpubkey
						);
					}
				},
			}
		...
		if ret.is_ok()
			&& self.funding_spend_seen
			&& self.latest_update_id != CLOSED_CHANNEL_UPDATE_ID
		{
				...
				ClaimEvent::BumpCommitment {
					...
					let commitment_tx_fee_satoshis = self.channel_value_satoshis
						- commitment_tx
							.output
							.iter()
							.fold(0u64, |sum, output| sum + output.value.to_sat());
	...
	fn sign_to_local_justice_tx(
		...
		justice_tx.input[input_idx]
			.witness
			.push_ecdsa_signature(&BitcoinSignature::sighash_all(sig));
	...
	fn check_spend_counterparty_transaction<L: Deref>(
		...
		L::Target: Logger,
	{
		...
		let commitment_number = 0xffffffffffff
			- ((((tx.input[0].sequence.0 as u64 & 0xffffff) << 3 * 8)
		...
		if commitment_number >= self.get_min_seen_secret() {
			...
			for (idx, outp) in tx.output.iter().enumerate() {
				if outp.script_pubkey == revokeable_p2wsh {
					...
					to_counterparty_output_info = Some((
						idx.try_into().expect("Txn can't have more than 2^32 outputs"),
						outp.value,
					));
				}
			}
			...
			if !claimable_outpoints.is_empty() || per_commitment_option.is_some() {
				// ie we're confident this is actually ours
				...
				if let Some(per_commitment_claimable_data) = per_commitment_option {
					fail_unbroadcast_htlcs!(
						...
						per_commitment_claimable_data.iter().map(|(htlc, htlc_source)| (
							htlc,
							htlc_source.as_ref().map(|htlc_source| htlc_source.as_ref())
						)),
						logger
					);
				} else {
			...
			log_info!(
				logger,
				"Got broadcast of non-revoked counterparty commitment transaction {}",
				commitment_txid
			);
			fail_unbroadcast_htlcs!(
				self,
			...
			let (htlc_claim_reqs, counterparty_output_info) = self
	...
	fn check_spend_counterparty_htlc<L: Deref>(
		&mut self, tx: &Transaction, commitment_number: u64, commitment_txid: &Txid, height: u32,
		logger: &L,
	) -> (Vec<PackageTemplate>, Option<TransactionOutputs>)
	where
		L::Target: Logger,
	{
		let secret = if let Some(secret) = self.get_secret(commitment_number) {
			secret
		} else {
			return (Vec::new(), None);
		};
		...
		for (idx, input) in tx.input.iter().enumerate() {
			if input.previous_output.txid == *commitment_txid
				&& input.witness.len() == 5
				&& tx.output.get(idx).is_some()
			{
				log_error!(
					logger,
					"Got broadcast of revoked counterparty HTLC transaction, spending {}:{}",
					htlc_txid,
					idx
				);
				let revk_outp = RevokedOutput::build(
	...
	fn get_broadcasted_holder_claims(
		&self, holder_tx: &HolderSignedTx, conf_height: u32,
	...
	fn get_broadcasted_holder_watch_outputs(
		&self, holder_tx: &HolderSignedTx, commitment_tx: &Transaction,
	) -> Vec<(u32, TxOut)> {
		...
		for &(ref htlc, _, _) in holder_tx.htlc_outputs.iter() {
			if let Some(transaction_output_index) = htlc.transaction_output_index {
				watch_outputs.push((
					transaction_output_index,
					commitment_tx.output[transaction_output_index as usize].clone(),
				));
			}
		}
		watch_outputs
		...
		L::Target: Logger,
	{
		...
		if self.current_holder_commitment_tx.txid == commitment_txid {
			...
			let res =
				self.get_broadcasted_holder_claims(&self.current_holder_commitment_tx, height);
			let mut to_watch =
				self.get_broadcasted_holder_watch_outputs(&self.current_holder_commitment_tx, tx);
			...
			fail_unbroadcast_htlcs!(
				...
				self.current_holder_commitment_tx
					.htlc_outputs
					.iter()
					.map(|(htlc, _, htlc_source)| (htlc, htlc_source.as_ref())),
				logger
			);
		...
		L::Target: Logger,
	{
		for (counterparty_commitment_txid, _) in &self.counterparty_commitment_txn_on_chain {
			...
			for (htlc, _) in self
				.counterparty_claimable_outpoints
				.get(counterparty_commitment_txid)
				.unwrap_or(&vec![])
			{
				log_trace!(
					logger,
					"Canceling claims for previously confirmed counterparty commitment {}",
		...
		if self.holder_tx_signed {
			...
			if self.current_holder_commitment_tx.txid != *confirmed_commitment_txid {
				log_trace!(
					logger,
					"Canceling claims for previously broadcast holder commitment {}",
					self.current_holder_commitment_tx.txid
				);
				let mut outpoint =
					BitcoinOutPoint { txid: self.current_holder_commitment_tx.txid, vout: 0 };
			...
			if let Some(prev_holder_commitment_tx) = &self.prev_holder_signed_commitment_tx {
				if prev_holder_commitment_tx.txid != *confirmed_commitment_txid {
					...
					let mut outpoint =
						BitcoinOutPoint { txid: prev_holder_commitment_tx.txid, vout: 0 };
		...
		L::Target: Logger,
	{
		...
		let commitment_tx =
			self.onchain_tx_handler.get_fully_signed_copy_holder_tx(&self.funding_redeemscript);
		...
		for htlc in self.current_holder_commitment_tx.htlc_outputs.iter() {
			if let Some(vout) = htlc.0.transaction_output_index {
				let preimage = if !htlc.0.offered {
					if let Some(preimage) = self.payment_preimages.get(&htlc.0.payment_hash) {
						Some(preimage.clone())
					} else {
						// We can't build an HTLC-Success transaction without the preimage
						continue;
				...
				if let Some(htlc_tx) = self
					.onchain_tx_handler
					.get_maybe_signed_htlc_tx(&::bitcoin::OutPoint { txid, vout }, &preimage)
				{
	...
	fn best_block_updated<B: Deref, F: Deref, L: Deref>(
		&mut self, header: &Header, height: u32, broadcaster: B,
		fee_estimator: &LowerBoundedFeeEstimator<F>, logger: &WithChannelMonitor<L>,
	) -> Vec<TransactionOutputs>
	where
		B::Target: BroadcasterInterface,
		...
		L::Target: Logger,
	{
		...
		if height > self.best_block.height {
			...
			log_trace!(
				logger,
				"Best block re-orged, replaced with new block {} at height {}",
				block_hash,
				height
			);
		...
		L::Target: Logger,
	{
		...
		for tx in &txn_matched {
			...
			for out in tx.output.iter() {
				if out.value > Amount::MAX_MONEY {
					panic!("Value-overflowing transaction provided to block connected");
				}
		...
		'tx_iter: for tx in &txn_matched {
			...
			if tx.input.len() == 1 {
				...
				if prevout.txid == self.funding_info.0.txid
					&& prevout.vout == self.funding_info.0.index as u32
				{
					let mut balance_spendable_csv = None;
					log_info!(
						logger,
						"Channel {} closed by funding output spend in txid {}.",
						&self.channel_id(),
						txid
					);
					...
					{
						if let Some((mut new_outpoints, new_outputs)) =
							self.check_spend_holder_transaction(&tx, height, &block_hash, &logger)
						{
							...
							let (mut new_outpoints, counterparty_output_idx_sats) = self
								.check_spend_counterparty_transaction(
									&tx,
									height,
									&block_hash,
									&logger,
			...
			if tx.input.len() >= 1 {
				...
				for tx_input in &tx.input {
					...
					if let Some(&commitment_number) =
						self.counterparty_commitment_txn_on_chain.get(&commitment_txid)
					{
						let (mut new_outpoints, new_outputs_option) = self
	...
	fn block_confirmed<B: Deref, F: Deref, L: Deref>(
		&mut self, conf_height: u32, conf_hash: BlockHash, txn_matched: Vec<&Transaction>,
		mut watch_outputs: Vec<TransactionOutputs>, mut claimable_outpoints: Vec<PackageTemplate>,
		broadcaster: &B, fee_estimator: &LowerBoundedFeeEstimator<F>,
		logger: &WithChannelMonitor<L>,
	) -> Vec<TransactionOutputs>
	where
		B::Target: BroadcasterInterface,
		...
		L::Target: Logger,
	{
		log_trace!(
			logger,
			"Processing {} matched transactions for block at height {}.",
			txn_matched.len(),
			conf_height
		);
		...
		if should_broadcast {
			let (mut new_outpoints, mut new_outputs) =
				self.generate_claimable_outpoints_and_watch_outputs(ClosureReason::HTLCsTimedOut);
			claimable_outpoints.append(&mut new_outpoints);
		...
		for entry in onchain_events_reaching_threshold_conf.drain(..) {
			match entry.event {
				...
				OnchainEvent::MaturingOutput { descriptor } => {
					log_debug!(
						logger,
						"Descriptor {} has got enough confirmations to be passed upstream",
						log_spendable!(descriptor)
					);
				...
				OnchainEvent::FundingSpendConfirmation {
					...
					self.confirmed_commitment_tx_counterparty_output =
						commitment_tx_to_counterparty_output;
				},
			}
		}
			...
			for tx in &txn_matched {
				if let Some(outputs) = self.get_outputs_to_watch().get(&tx.compute_txid()) {
					for idx_and_script in outputs.iter() {
						...
						assert_eq!(
							tx.output[idx_and_script.0 as usize].script_pubkey,
							idx_and_script.1
						);
					}
				}
	...
	fn block_disconnected<B: Deref, F: Deref, L: Deref>(
		&mut self, header: &Header, height: u32, broadcaster: B, fee_estimator: F,
		logger: &WithChannelMonitor<L>,
	) where
		B::Target: BroadcasterInterface,
		F::Target: FeeEstimator,
		...
		L::Target: Logger,
	{
		...
		if let Some(removed_height) = removed_height {
			log_info!(
				logger,
				"transaction_unconfirmed of txid {} implies height {} was reorg'd out",
				txid,
				removed_height
			);
			self.onchain_events_awaiting_threshold_conf.retain(|ref entry| {
				if entry.height >= removed_height {
					...
					if *idx == input.previous_output.vout {
						#[cfg(test)]
						{
							...
							if _script_pubkey.is_p2wsh() {
								if input.witness.last().unwrap().to_vec()
									== deliberately_bogus_accepted_htlc_witness_program()
								{
								...
								assert_eq!(
									&bitcoin::Address::p2wsh(
										&ScriptBuf::from(input.witness.last().unwrap().to_vec()),
		...
		L::Target: Logger,
	{
		...
		scan_commitment!(
			self.current_holder_commitment_tx.htlc_outputs.iter().map(|&(ref a, _, _)| a),
			true
		);
		...
		'outer_loop: for input in &tx.input {
			...
			macro_rules! check_htlc_valid_counterparty {
				($htlc_output: expr, $per_commitment_data: expr) => {
					for &(ref pending_htlc, ref pending_source) in $per_commitment_data {
						if pending_htlc.payment_hash == $htlc_output.payment_hash
							&& pending_htlc.amount_msat == $htlc_output.amount_msat
						{
							if let &Some(ref source) = pending_source {
								...
								payment_data = Some((
									(**source).clone(),
									$htlc_output.payment_hash,
									$htlc_output.amount_msat,
									$htlc_output.amount_rgb,
								));
								break;
							}
						}
					...
					for (ref htlc_output, source_option) in $htlcs {
						if Some(input.previous_output.vout) == htlc_output.transaction_output_index
						{
							if let Some(ref source) = source_option {
								...
								if let Some(current_counterparty_commitment_txid) =
									&self.current_counterparty_commitment_txid
								{
									check_htlc_valid_counterparty!(
										htlc_output,
										self.counterparty_claimable_outpoints
											.get(current_counterparty_commitment_txid)
											.unwrap()
									);
								}
								if payment_data.is_none() {
									if let Some(prev_counterparty_commitment_txid) =
										&self.prev_counterparty_commitment_txid
									{
										check_htlc_valid_counterparty!(
											htlc_output,
											self.counterparty_claimable_outpoints
												.get(prev_counterparty_commitment_txid)
												.unwrap()
										);
									}
								}
							...
							if payment_data.is_none() {
								...
								self.onchain_events_awaiting_threshold_conf.push(
									OnchainEventEntry {
										...
										event: OnchainEvent::HTLCSpendConfirmation {
											...
											preimage: if accepted_preimage_claim
												|| offered_preimage_claim
											{
			...
			if input.previous_output.txid == self.current_holder_commitment_tx.txid {
				scan_commitment!(
					...
					"our latest holder commitment tx",
					true
				);
			}
			if let Some(ref prev_holder_signed_commitment_tx) =
				...
				if input.previous_output.txid == prev_holder_signed_commitment_tx.txid {
					scan_commitment!(
						prev_holder_signed_commitment_tx
							...
							.iter()
							.map(|&(ref a, _, ref b)| (a, b.as_ref())),
						"our previous holder commitment tx",
						true
					);
				}
			}
			if let Some(ref htlc_outputs) =
				self.counterparty_claimable_outpoints.get(&input.previous_output.txid)
			{
				scan_commitment!(
					htlc_outputs
						.iter()
		...
		for (i, outp) in tx.output.iter().enumerate() {
				...
				self.broadcasted_holder_revokable_script
			{
				if broadcasted_holder_revokable_script.0 == outp.script_pubkey {
					spendable_outputs.push(SpendableOutputDescriptor::DelayedPaymentOutput(
						DelayedPaymentOutputDescriptor {
							...
							channel_transaction_parameters: Some(
								self.onchain_tx_handler.channel_transaction_parameters.clone(),
							),
						},
					));
				}
			...
			if self.counterparty_payment_script == outp.script_pubkey {
				spendable_outputs.push(SpendableOutputDescriptor::StaticPaymentOutput(
					StaticPaymentOutputDescriptor {
						outpoint: OutPoint { txid: tx.compute_txid(), index: i as u16 },
						output: outp.clone(),
						channel_keys_id: self.channel_keys_id,
		...
		L::Target: Logger,
	{
		for spendable_output in self.get_spendable_outputs(tx) {
			...
			log_info!(
				logger,
				"Received spendable output {}, spendable at height {}",
				log_spendable!(spendable_output),
...
{
	fn read<R: io::Read>(
		reader: &mut R, args: (&'a ES, &'b SP, PathBuf),
		...
		let outpoint = OutPoint { txid: Readable::read(reader)?, index: Readable::read(reader)? };
			...
			if first_idx == 0 {
				...
				if second_point_slice[0..32] == [0; 32] && second_point_slice[32] == 0 {
					...
					Some((
						first_idx,
						first_point,
						Some(unwrap_obj!(PublicKey::from_slice(&second_point_slice))),
					))
				}
			}
		};
		...
		let mut counterparty_claimable_outpoints = hash_map_with_capacity(cmp::min(
			counterparty_claimable_outpoints_len as usize,
			MAX_ALLOC_SIZE / 64,
		));
		for _ in 0..counterparty_claimable_outpoints_len {
			...
			for _ in 0..htlcs_count {
				htlcs.push((
					read_htlc_in_commitment!(),
					<Option<HTLCSource> as Readable>::read(reader)?
						.map(|o: HTLCSource| Box::new(o)),
				));
			}
		...
		let mut counterparty_commitment_txn_on_chain = hash_map_with_capacity(cmp::min(
			counterparty_commitment_txn_on_chain_len as usize,
			MAX_ALLOC_SIZE / 32,
		));
		...
		let mut counterparty_hash_commitment_number = hash_map_with_capacity(cmp::min(
			counterparty_hash_commitment_number_len as usize,
			MAX_ALLOC_SIZE / 32,
		));
		for _ in 0..counterparty_hash_commitment_number_len {
			...
			let commitment_number = <U48 as Readable>::read(reader)?.0;
			if let Some(_) =
				counterparty_hash_commitment_number.insert(payment_hash, commitment_number)
			{
				return Err(DecodeError::InvalidValue);
			}
		}
		...
		let mut payment_preimages =
			hash_map_with_capacity(cmp::min(payment_preimages_len as usize, MAX_ALLOC_SIZE / 32));
		...
		let mut pending_monitor_events = Some(Vec::with_capacity(cmp::min(
			pending_monitor_events_len as usize,
			MAX_ALLOC_SIZE / (32 + 8 * 3),
		)));
		...
		let mut pending_events = Vec::with_capacity(cmp::min(
			pending_events_len as usize,
			MAX_ALLOC_SIZE / mem::size_of::<Event>(),
		));
		...
		let mut outputs_to_watch = hash_map_with_capacity(cmp::min(
			outputs_to_watch_len as usize,
			MAX_ALLOC_SIZE
				/ (mem::size_of::<Txid>()
					+ mem::size_of::<u32>()
					+ mem::size_of::<Vec<ScriptBuf>>()),
		));
		for _ in 0..outputs_to_watch_len {
			let txid = Readable::read(reader)?;
			let outputs_len: u64 = Readable::read(reader)?;
			let mut outputs = Vec::with_capacity(cmp::min(
				outputs_len as usize,
				MAX_ALLOC_SIZE / (mem::size_of::<u32>() + mem::size_of::<ScriptBuf>()),
			));
		...
		if let Some(ref mut pending_monitor_events) = pending_monitor_events {
			if pending_monitor_events
				.iter()
				.any(|e| matches!(e, MonitorEvent::HolderForceClosed(_)))
				&& pending_monitor_events
					.iter()
					.any(|e| matches!(e, MonitorEvent::HolderForceClosedWithInfo { .. }))
			{
		...
		{
			let payment_point =
				onchain_tx_handler.channel_transaction_parameters.holder_pubkeys.payment_point;
			counterparty_payment_script =
		...
		Ok((
			...
			ChannelMonitor::from_impl(ChannelMonitorImpl {
				latest_update_id,
				commitment_transaction_number_obscure_factor,
...
mod tests {
	...
	use crate::{
		...
		get_route_and_payment_hash, unwrap_send_err,
	};
	...
	fn do_test_funding_spend_refuses_updates(use_local_txn: bool) {
		...
		let (_, pre_update_monitor) = <(BlockHash, ChannelMonitor<InMemorySigner>)>::read(
			...
			(&nodes[1].keys_manager.backing, &nodes[1].keys_manager.backing),
		)
		.unwrap();
		...
		let mut replay_update =
			monitor_updates.get(&channel.2).unwrap().iter().rev().next().unwrap().clone();
		assert_eq!(replay_update.updates.len(), 1);
		if let ChannelMonitorUpdateStep::LatestCounterpartyCommitmentTXInfo { .. } =
			replay_update.updates[0]
		{
		} else {
			panic!();
		}
	...
	fn test_prune_preimages() {
		...
		macro_rules! preimages_slice_to_htlc_outputs {
			($preimages_slice: expr) => {
				preimages_slice_to_htlcs!($preimages_slice)
					.into_iter()
					.map(|(htlc, _)| (htlc, None))
					.collect()
			};
		}
		...
		let monitor = ChannelMonitor::new(
			Secp256k1::new(),
			keys,
			Some(ShutdownScript::new_p2wpkh_from_pubkey(shutdown_pubkey).into_inner()),
			0,
			&ScriptBuf::new(),
			...
			46,
			0,
			HolderCommitmentTransaction::dummy(&mut Vec::new()),
			best_block,
			dummy_key,
			channel_id,
		);
		...
		monitor.provide_latest_counterparty_commitment_tx(
			Txid::from_byte_array(Sha256::hash(b"2").to_byte_array()),
		...
		monitor.provide_latest_counterparty_commitment_tx(
			Txid::from_byte_array(Sha256::hash(b"3").to_byte_array()),
		...
		monitor.provide_latest_counterparty_commitment_tx(
			Txid::from_byte_array(Sha256::hash(b"4").to_byte_array()),
	...
	fn test_claim_txn_weight_computation() {
		...
		use crate::ln::channel_keys::{HtlcBasepoint, HtlcKey};
		macro_rules! sign_input {
			($sighash_parts: expr, $idx: expr, $amount: expr, $weight: expr, $sum_actual_sigs: expr, $opt_anchors: expr) => {
				let htlc = HTLCOutputInCommitment {
					offered: if *$weight == weight_revoked_offered_htlc($opt_anchors)
						|| *$weight == weight_offered_htlc($opt_anchors)
					{
						true
					} else {
						false
				...
				let redeem_script = if *$weight == WEIGHT_REVOKED_OUTPUT {
					chan_utils::get_revokeable_redeemscript(
						&RevocationKey::from_basepoint(
							&secp_ctx,
							&RevocationBasepoint::from(pubkey),
							&pubkey,
						),
						256,
						&DelayedPaymentKey::from_basepoint(
							&secp_ctx,
							&DelayedPaymentBasepoint::from(pubkey),
							&pubkey,
						),
					)
				} else {
					chan_utils::get_htlc_redeemscript_with_explicit_keys(
						&htlc,
						$opt_anchors,
						&HtlcKey::from_basepoint(&secp_ctx, &HtlcBasepoint::from(pubkey), &pubkey),
						&HtlcKey::from_basepoint(&secp_ctx, &HtlcBasepoint::from(pubkey), &pubkey),
						&RevocationKey::from_basepoint(
							&secp_ctx,
							&RevocationBasepoint::from(pubkey),
				...
				let sighash = hash_to_message!(
					&$sighash_parts
						.p2wsh_signature_hash($idx, &redeem_script, $amount, EcdsaSighashType::All)
						.unwrap()[..]
				);
				...
				if *$weight == WEIGHT_REVOKED_OUTPUT {
					witness.push(vec![1]);
				} else if *$weight == weight_revoked_offered_htlc($opt_anchors)
					|| *$weight == weight_revoked_received_htlc($opt_anchors)
				{
		...
		let txid =
			Txid::from_str("56944c5d3f98413ef45cf54545538103cc9f298e0575820ad3591376e2e0f65d")
				.unwrap();
		...
		.iter()
		{
			...
			for i in 0..4 {
				claim_tx.input.push(TxIn {
					previous_output: BitcoinOutPoint { txid, vout: i },
			...
			claim_tx
				.output
				.push(TxOut { script_pubkey: script_pubkey.clone(), value: Amount::ZERO });
			let base_weight = claim_tx.weight().to_wu();
			let inputs_weight = vec![
			...
			assert_eq!(
				base_weight + inputs_total_weight,
				claim_tx.weight().to_wu() + /* max_length_sig */ (73 * inputs_weight.len() as u64 - sum_actual_sigs)
			);
		}
		...
		.iter()
		{
			...
			let base_weight = claim_tx.weight().to_wu();
			let inputs_weight = vec![
				weight_offered_htlc(channel_type_features),
				weight_received_htlc(channel_type_features),
				weight_received_htlc(channel_type_features),
				weight_received_htlc(channel_type_features),
			];
			let mut inputs_total_weight = 2; // count segwit flags
		...
		.iter()
		{
			...
			let mut sum_actual_sigs = 0;
			claim_tx.input.push(TxIn {
				previous_output: BitcoinOutPoint { txid, vout: 0 },
				script_sig: ScriptBuf::new(),
				sequence: Sequence::ENABLE_RBF_NO_LOCKTIME,
				witness: Witness::new(),
			});
			...
			assert_eq!(
				base_weight + inputs_total_weight,
				claim_tx.weight().to_wu() + /* max_length_isg */ (73 * inputs_weight.len() as u64 - sum_actual_sigs)
			);
		}
	}
	...
	fn test_with_channel_monitor_impl_logger() {
		...
		logger.assert_log_context_contains(
			"lightning::chain::channelmonitor::tests",
			Some(dummy_key),
			Some(chan_id),
			6,
		);
