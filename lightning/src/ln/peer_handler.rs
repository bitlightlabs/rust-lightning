...
use crate::ln::msgs;
use crate::ln::msgs::{
	ChannelMessageHandler, Init, LightningError, OnionMessageHandler, RoutingMessageHandler,
	SocketAddress,
};
	...
	NS::Target: NodeSigner,
{
	...
	> {
		...
		let message = match self.do_handle_message_holding_peer_lock(
			peer_lock,
			message,
			&their_node_id,
			&logger,
...
mod tests {
	...
	use crate::ln::peer_handler::{
		filter_addresses, CustomMessageHandler, ErroringMessageHandler, IgnoringMessageHandler,
		MessageHandler, PeerManager, SocketDescriptor, MAX_BUFFER_DRAIN_TICK_INTERVALS_PER_PEER,
	};
	use crate::ln::types::ChannelId;
