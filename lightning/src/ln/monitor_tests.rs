...
use crate::chain::channelmonitor::{
	Balance, BalanceSource, ANTI_REORG_DELAY, LATENCY_GRACE_PERIOD_BLOCKS,
};
...
fn chanmon_fail_from_stale_commitment() {
	...
	nodes[0].node.handle_update_fail_htlc(
		...
		&fail_updates.update_fail_htlcs[0],
	);
	...
	expect_payment_failed_with_update!(
		...
		payment_hash,
		false,
		update_a.contents.short_channel_id,
		true
	);
}

fn test_spendable_output<'a, 'b, 'c, 'd>(
	node: &'a Node<'b, 'c, 'd>, spendable_tx: &Transaction, has_anchors_htlc_event: bool,
...
fn archive_fully_resolved_monitors() {
	...
	let node_chanmgrs =
		create_node_chanmgrs(2, &node_cfgs, &[Some(user_config), Some(user_config)]);
...
fn do_chanmon_claim_value_coop_close(anchors: bool) {
	...
	let commitment_tx_fee =
		chan_feerate * chan_utils::commitment_tx_base_weight(&channel_type_features) / 1000;
	...
	assert_eq!(
		vec![Balance::ClaimableAwaitingConfirmations {
			amount_satoshis: 1_000_000 - 1_000 - commitment_tx_fee - anchor_outputs_value,
	...
	assert!(get_monitor!(nodes[0], chan_id)
		.get_spendable_outputs(&shutdown_tx[0], shutdown_tx_conf_height_a)
		.is_empty());
	assert!(get_monitor!(nodes[1], chan_id)
		.get_spendable_outputs(&shutdown_tx[0], shutdown_tx_conf_height_b)
		.is_empty());
...
fn do_test_claim_value_force_close(anchors: bool, prev_commitment_tx: bool) {
	...
	let (timeout_payment_preimage, timeout_payment_hash, ..) =
		route_payment(&nodes[0], &[&nodes[1]], 4_000_200);
	...
	let commitment_tx_fee = chan_feerate as u64
		* (chan_utils::commitment_tx_base_weight(&channel_type_features)
			+ 2 * chan_utils::COMMITMENT_TX_WEIGHT_PER_HTLC)
		/ 1000;
		...
		sorted_vec(vec![
			Balance::ClaimableOnChannelClose {
				amount_satoshis: 1_000_000
					- 3_000 - 4_000 - 1_000
					- 3 - commitment_tx_fee
					- anchor_outputs_value
					- 1, /* msat amount that is burned to fees */
				transaction_fee_satoshis: commitment_tx_fee,
	...
	assert_eq!(
		sorted_vec(vec![
			...
			received_htlc_balance.clone(),
			received_htlc_timeout_balance.clone()
		]),
		sorted_vec(
			nodes[1]
				.chain_monitor
	...
	if prev_commitment_tx {
		...
		nodes[0].node.handle_update_fulfill_htlc(
			&nodes[1].node.get_our_node_id(),
			&b_htlc_msgs.update_fulfill_htlcs[0],
		);
		expect_payment_sent(&nodes[0], payment_preimage, None, false, false);
		nodes[0].node.handle_commitment_signed(
			&nodes[1].node.get_our_node_id(),
			&b_htlc_msgs.commitment_signed,
		);
		check_added_monitors!(nodes[0], 1);
	...
	let commitment_tx_fee = chan_feerate as u64
		* (chan_utils::commitment_tx_base_weight(&channel_type_features)
			+ if prev_commitment_tx { 1 } else { 2 } * chan_utils::COMMITMENT_TX_WEIGHT_PER_HTLC)
		/ 1000;
	let mut a_expected_balances = vec![
		Balance::ClaimableOnChannelClose {
	...
	if anchors {
		...
		match events.pop().unwrap() {
			Event::BumpTransaction(bump_event) => {
				...
				if let BumpTransactionEvent::HTLCResolution { ref mut htlc_descriptors, .. } =
					&mut first_htlc_event
				{
	...
	assert_eq!(
		remote_txn[0].output[b_broadcast_txn[0].input[0].previous_output.vout as usize]
			.value
			.to_sat(),
		3_000
	);
	assert_eq!(
		remote_txn[0].output[b_broadcast_txn[1].input[0].previous_output.vout as usize]
			.value
			.to_sat(),
		4_000
	);
	...
	assert_eq!(
		sorted_vec(vec![
			...
			// long.
			received_htlc_claiming_balance.clone(),
	...
	assert_eq!(
		sorted_vec(vec![sent_htlc_balance.clone(), sent_htlc_timeout_balance.clone()]),
		sorted_vec(
			nodes[0]
				.chain_monitor
				.chain_monitor
	...
	connect_blocks(&nodes[0], ANTI_REORG_DELAY - 1);
	assert_eq!(
		vec![sent_htlc_timeout_balance.clone()],
		nodes[0]
			.chain_monitor
			.chain_monitor
			.get_monitor(funding_outpoint)
	...
	assert_ne!(
		a_broadcast_txn[0].input[0].previous_output.vout,
		a_broadcast_txn[1].input[0].previous_output.vout
	);
	// a_broadcast_txn [0] and [1] should spend the HTLC outputs of the commitment tx
	assert_eq!(
		remote_txn[0].output[a_broadcast_txn[0].input[0].previous_output.vout as usize]
			.value
			.to_sat(),
		3_000
	);
	assert_eq!(
		remote_txn[0].output[a_broadcast_txn[1].input[0].previous_output.vout as usize]
			.value
			.to_sat(),
		4_000
	);
	...
	assert_eq!(
		vec![received_htlc_timeout_claiming_balance.clone()],
		nodes[1]
			.chain_monitor
			.chain_monitor
			.get_monitor(funding_outpoint)
...
fn do_test_balances_on_local_commitment_htlcs(anchors: bool) {
	...
	let (route_2, payment_hash_2, payment_preimage_2, payment_secret_2) =
		get_route_and_payment_hash!(nodes[0], nodes[1], 20_000_000);
	nodes[0]
		.node
		.send_payment_with_route(
			route_2,
	...
	assert_eq!(
		sorted_vec(vec![
			Balance::ClaimableAwaitingConfirmations {
				amount_satoshis: 1_000_000
					- 10_000 - 20_000 - commitment_tx_fee
					- anchor_outputs_value,
				confirmation_height: node_a_commitment_claimable,
				source: BalanceSource::HolderForceClosed,
			},
			htlc_balance_known_preimage.clone(),
	...
	assert!(get_monitor!(nodes[0], chan_id)
		.get_spendable_outputs(&commitment_tx, commitment_tx_conf_height_a)
		.is_empty());
	connect_blocks(&nodes[0], 1);
	assert_eq!(
		vec![Balance::ClaimableAwaitingConfirmations {
...
fn test_no_preimage_inbound_htlc_balances() {
	...
	assert_eq!(
		sorted_vec(vec![
			...
			a_received_htlc_balance.clone(),
			a_sent_htlc_balance.clone()
		]),
		sorted_vec(
			nodes[0]
				.chain_monitor
	...
	assert_eq!(
		sorted_vec(vec![
			...
			b_received_htlc_balance.clone(),
			b_sent_htlc_balance.clone()
		]),
		sorted_vec(
			nodes[1]
				.chain_monitor
	...
	let node_a_commitment_claimable = nodes[0].best_block_info().1 + BREAKDOWN_TIMEOUT as u32;
	let as_pre_spend_claims = sorted_vec(vec![
		Balance::ClaimableAwaitingConfirmations {
			amount_satoshis: 1_000_000
				- 500_000 - 10_000
				- chan_feerate
	...
	let mut bs_pre_spend_claims = sorted_vec(vec![
		Balance::ClaimableAwaitingConfirmations {
			amount_satoshis: 500_000 - 20_000,
			confirmation_height: node_b_commitment_claimable,
			source: BalanceSource::CounterpartyForceClosed,
	...
	bs_pre_spend_claims.retain(|e| {
		if let Balance::ClaimableAwaitingConfirmations { .. } = e {
			false
		} else {
			true
		}
	...
	assert_eq!(
		vec![b_received_htlc_balance.clone()],
		nodes[1]
			.chain_monitor
			.chain_monitor
			.get_monitor(funding_outpoint)
...
fn do_test_revoked_counterparty_commitment_balances(anchors: bool, confirm_htlc_spend_first: bool) {
	...
	let (claimed_payment_preimage, claimed_payment_hash, ..) =
		route_payment(&nodes[0], &[&nodes[1]], 3_000_100);
	...
	events.retain(|ev| match ev {
		Event::HTLCHandlingFailed {
			failed_next_destination: HTLCDestination::NextHopChannel { node_id, channel_id },
			..
		} => {
		...
		Event::HTLCHandlingFailed {
			failed_next_destination: HTLCDestination::FailedPayment { payment_hash },
			..
		} => {
	...
	if let Event::PendingHTLCsForwardable { .. } = events[0] {
	} else {
		panic!();
	}
	match &events[1] {
	...
	let mut claim_txn: Vec<_> = nodes[1]
		.tx_broadcaster
		.txn_broadcasted
		.lock()
		.unwrap()
		.drain(..)
		.filter(|tx| {
			tx.input.iter().any(|inp| inp.previous_output.txid == as_revoked_txn[0].compute_txid())
		})
		.collect();
	// Currently the revoked commitment is claimed in four transactions as the HTLCs all expire
	// quite soon.
	assert_eq!(claim_txn.len(), 4);
	claim_txn.sort_unstable_by_key(|tx| {
		tx.output.iter().map(|output| output.value.to_sat()).sum::<u64>()
	});
	...
	let commitment_tx_fee = chan_feerate
		* (chan_utils::commitment_tx_base_weight(&channel_type_features)
			+ 3 * chan_utils::COMMITMENT_TX_WEIGHT_PER_HTLC)
		/ 1000;
	let anchor_outputs_value = if anchors { channel::ANCHOR_OUTPUT_VALUE_SATOSHI * 2 } else { 0 };
	...
	let expected_balance_b = vec![
		...
		Balance::CounterpartyRevokedOutputClaimable { amount_satoshis: 3_000 },
		Balance::CounterpartyRevokedOutputClaimable { amount_satoshis: 4_000 },
	];
	...
	let largest_htlc_unclaimed_balance =
		Balance::CounterpartyRevokedOutputClaimable { amount_satoshis: 5_000 };
	...
	assert_eq!(
		sorted_vec_with_additions(
			&expected_balance_b,
			&[&to_self_unclaimed_balance, &largest_htlc_unclaimed_balance]
		),
		sorted_vec(
			nodes[1]
				.chain_monitor
	...
	let to_self_claimed_balance = Balance::ClaimableAwaitingConfirmations {
		amount_satoshis: 1_000_000
			...
			- anchor_outputs_value
			- to_self_claim_fee
			- 1, /* rounded up msat parts of HTLCs */
	...
	if confirm_htlc_spend_first {
		...
		assert_eq!(
			sorted_vec_with_additions(
				&expected_balance_b,
				&[&to_self_claimed_balance, &largest_htlc_unclaimed_balance]
			),
			sorted_vec(
				nodes[1]
					.chain_monitor
	...
	assert_eq!(
		sorted_vec_with_additions(
			&expected_balance_b,
			&[&to_self_claimed_balance, &largest_htlc_claimed_balance]
		),
		sorted_vec(
			nodes[1]
				.chain_monitor
	...
	assert_eq!(
		sorted_vec(vec![
			...
			Balance::ClaimableAwaitingConfirmations {
				amount_satoshis: 1_000_000
					- 100_000 - 3_000 - commitment_tx_fee
					- anchor_outputs_value
					- to_self_claim_fee - 1, /* rounded up msat parts of HTLCs */
				confirmation_height: to_self_claimed_avail_height,
				source: BalanceSource::CounterpartyForceClosed,
			},
			Balance::ClaimableAwaitingConfirmations {
	...
	expect_payment_failed_conditions_event(
		payment_failed_events[..2].to_vec(),
		missing_htlc_payment_hash,
		false,
	...
	test_spendable_output(
		&nodes[1],
		&claim_txn[if confirm_htlc_spend_first { 2 } else { 3 }],
		false,
	);
	connect_blocks(&nodes[1], 1);
	test_spendable_output(
		&nodes[1],
		&claim_txn[if confirm_htlc_spend_first { 3 } else { 2 }],
		false,
	);
...
fn do_test_revoked_counterparty_htlc_tx_balances(anchors: bool) {
	...
	let revoked_htlc_success = {
		...
		assert_eq!(
			txn[0].input[0].witness.last().unwrap().len(),
	...
	assert_ne!(
		revoked_htlc_success.input[0].previous_output,
		revoked_htlc_timeout.input[0].previous_output
	);
	...
	let commitment_tx_fee = chan_feerate
		* (chan_utils::commitment_tx_base_weight(&channel_type_features)
		...
		/ 1000;
	let anchor_outputs_value = if anchors { channel::ANCHOR_OUTPUT_VALUE_SATOSHI * 2 } else { 0 };
	let as_balances = sorted_vec(vec![
		Balance::ClaimableAwaitingConfirmations {
			// to_remote output in B's revoked commitment
			amount_satoshis: 1_000_000
				- 12_000 - 3_000 - commitment_tx_fee
				- anchor_outputs_value
				- 1, /* The rounded up msat part of the one HTLC */
	...
	let as_revoked_htlc_success_claim_fee =
		chan_feerate * as_htlc_claim_tx[0].weight().to_wu() / 1000;
	if anchors {
		// With anchors, B can pay for revoked_htlc_success's fee with additional inputs, rather
		// than with the HTLC itself.
		fuzzy_assert_eq(
			as_htlc_claim_tx[0].output[0].value.to_sat(),
	...
	connect_blocks(
		&nodes[0],
		revoked_htlc_timeout.lock_time.to_consensus_u32() - nodes[0].best_block_info().1,
	);
...
fn do_test_revoked_counterparty_aggregated_claims(anchors: bool) {
	...
	let (claimed_payment_preimage, claimed_payment_hash, ..) =
		route_payment(&nodes[1], &[&nodes[0]], 3_000_100);
	...
	get_monitor!(nodes[0], chan_id).provide_payment_preimage(
		&claimed_payment_hash,
		&claimed_payment_preimage,
	...
	if anchors {
		assert_eq!(
			vec![
				...
				Balance::ClaimableAwaitingConfirmations {
					amount_satoshis: revoked_to_self_claim.as_ref().unwrap().output[0]
						.value
						.to_sat(),
					confirmation_height: rest_claim_maturity,
					source: BalanceSource::CounterpartyForceClosed,
	...
	if anchors {
		...
		for (i, event) in events.into_iter().enumerate() {
			if let Event::SpendableOutputs { outputs, .. } = event {
				...
				check_spends!(
					spend_tx,
					if i == 0 { &claim_txn_2[1] } else { revoked_to_self_claim.as_ref().unwrap() }
				);
			} else {
				panic!();
			}
...
fn do_test_claimable_balance_correct_while_payment_pending(outbound_payment: bool, anchors: bool) {
	...
	let node_chanmgrs = create_node_chanmgrs(
		3,
		&node_cfgs,
		&[Some(user_config), Some(user_config), Some(user_config)],
	);
	...
	let (_, _, chan_ab_id, funding_tx_ab) = create_announced_chan_between_nodes_with_value(
		&nodes, 0, 1, 1_000_000, /* channel_value (sat) */
		0,         /* push_msat */
	);
	...
	let (_, _, chan_bc_id, funding_tx_bc) = create_announced_chan_between_nodes_with_value(
		&nodes, 1, 2, 1_000_000, /* channel_value (sat) */
		0,         /* push_msat */
	);
	...
	let commitment_tx_fee = chan_feerate as u64
		* (chan_utils::commitment_tx_base_weight(&channel_type_features)
			+ chan_utils::COMMITMENT_TX_WEIGHT_PER_HTLC)
		/ 1000;
	...
	if outbound_payment {
		assert_eq!(
			...
			nodes[0]
				.chain_monitor
				.chain_monitor
				.get_monitor(funding_outpoint_ab)
				.unwrap()
				.get_claimable_balances()
				.iter()
				.map(|x| x.claimable_amount_satoshis())
				.sum()
		);
	} else {
		assert_eq!(
		...
		assert_eq!(
			...
			nodes[1]
				.chain_monitor
				.chain_monitor
				.get_monitor(funding_outpoint_bc)
				.unwrap()
				.get_claimable_balances()
				.iter()
				.map(|x| x.claimable_amount_satoshis())
...
fn do_test_monitor_rebroadcast_pending_claims(anchors: bool) {
	...
	let (_, _, _, chan_id, funding_tx) =
		create_chan_between_nodes_with_value(&nodes[0], &nodes[1], 1_000_000, 500_000_000);
	...
	assert_eq!(
		commitment_txn.len(),
		if anchors {
			1 /* commitment tx only */
		} else {
			2 /* commitment and htlc timeout tx */
		}
	);
	...
	let coinbase_tx = Transaction {
		...
		input: vec![TxIn { ..Default::default() }],
		output: vec![TxOut {
			// UTXO to attach fees to `htlc_tx` on anchors
			value: Amount::ONE_BTC,
			script_pubkey: nodes[0].wallet_source.get_change_script().unwrap(),
		}],
	};
	...
	*nodes[0].fee_estimator.sat_per_kw.lock().unwrap() *= 2;
	...
	if nodes[0].connect_style.borrow().updates_best_block_first() {
		...
		nodes[0]
			.wallet_source
			.remove_utxo(bitcoin::OutPoint { txid: htlc_tx.compute_txid(), vout: 1 });
		check_htlc_retry(true, anchors);
	}
	nodes[0].chain_monitor.chain_monitor.rebroadcast_pending_claims();
	check_htlc_retry(false, false);
...
fn do_test_yield_anchors_events(have_htlcs: bool) {
	...
	let node_chanmgrs =
		create_node_chanmgrs(2, &node_cfgs, &[Some(anchors_config), Some(anchors_config)]);
	let nodes = create_network(2, &node_cfgs, &node_chanmgrs);
	...
	if have_htlcs {
		nodes[0]
			...
			.lock()
			.unwrap()
			.insert(ConfirmationTarget::UrgentOnChainSweep, 500);
	} else {
		nodes[0]
			.fee_estimator
			.target_override
			.lock()
			.unwrap()
			.insert(ConfirmationTarget::OutputSpendingFee, 500);
	}
	...
	if have_htlcs {
		get_monitor!(nodes[0], chan_id).provide_payment_preimage(
			&payment_hash_2.unwrap(),
			&payment_preimage_2.unwrap(),
			&node_cfgs[0].tx_broadcaster,
			&LowerBoundedFeeEstimator::new(node_cfgs[0].fee_estimator),
			&nodes[0].logger,
		);
		get_monitor!(nodes[1], chan_id).provide_payment_preimage(
			&payment_hash_1.unwrap(),
			&payment_preimage_1.unwrap(),
			&node_cfgs[1].tx_broadcaster,
			&LowerBoundedFeeEstimator::new(node_cfgs[1].fee_estimator),
			&nodes[1].logger,
		);
	} else {
		nodes[0]
	...
	let (commitment_tx, anchor_tx) = match holder_events.pop().unwrap() {
		Event::BumpTransaction(event) => {
			let coinbase_tx = Transaction {
				...
				input: vec![TxIn { ..Default::default() }],
				output: vec![TxOut {
					// UTXO to attach fees to `anchor_tx`
					value: Amount::ONE_BTC,
					script_pubkey: nodes[0].wallet_source.get_change_script().unwrap(),
				}],
			};
	...
	if nodes[0].connect_style.borrow().updates_best_block_first() {
		...
		if let Event::BumpTransaction(BumpTransactionEvent::ChannelClose { .. }) =
			holder_events.remove(0)
		{
		} else {
			panic!("unexpected event");
		}
...
fn test_anchors_aggregated_revoked_htlc_tx() {
	...
	let chan_a =
		create_announced_chan_between_nodes_with_value(&nodes, 0, 1, 1_000_000, 20_000_000);
	let chan_b =
		create_announced_chan_between_nodes_with_value(&nodes, 0, 1, 1_000_000, 20_000_000);
	...
	reload_node!(
		...
		anchors_config,
		bob_serialized,
		&[&bob_serialized_monitor_a, &bob_serialized_monitor_b],
		bob_persister,
		bob_chain_monitor,
		bob_deserialized
	);
	...
	*nodes[1].fee_estimator.sat_per_kw.lock().unwrap() *= 2;
	...
	check_closed_event!(
		...
		[nodes[0].node.get_our_node_id(); 2],
		1000000
	);
	...
	for (idx, event) in events.into_iter().enumerate() {
		...
		nodes[1]
			.wallet_source
			.add_utxo(bitcoin::OutPoint { txid: coinbase_tx.compute_txid(), vout: 0 }, utxo_value);
		match event {
	...
	for node in &nodes {
		mine_transactions(
			node,
			&[
				&revoked_commitment_txs[0],
				&anchor_txs[0],
				&revoked_commitment_txs[1],
				&anchor_txs[1],
			],
		);
	}
	...
	// revoked outputs.
	{
		...
		let (revoked_htlc_claim_a, revoked_htlc_claim_b) =
			if txn[0].input[0].previous_output.txid == revoked_commitment_txs[0].compute_txid() {
				(
					if txn[0].input.len() == 2 { &txn[0] } else { &txn[1] },
					if txn[2].input.len() == 2 { &txn[2] } else { &txn[3] },
				)
			} else {
				(
					if txn[2].input.len() == 2 { &txn[2] } else { &txn[3] },
	...
	let htlc_tx = {
		...
		let coinbase_tx = Transaction {
			...
			input: vec![TxIn { ..Default::default() }],
			output: vec![TxOut {
				// UTXO to attach fees to `htlc_tx`
				value: Amount::ONE_BTC,
		...
		for (idx, htlc_descriptor) in descriptors.into_iter().enumerate() {
			...
			let our_sig = signer
				.sign_holder_htlc_transaction(&htlc_tx, htlc_input_idx, &htlc_descriptor, &secp)
				.unwrap();
			let witness_script = htlc_descriptor.witness_script(&secp);
			htlc_tx.input[htlc_input_idx].witness =
				htlc_descriptor.tx_input_witness(&our_sig, &witness_script);
		}
		let fee_utxo_sig = {
	...
	assert!(nodes[1].chain_monitor.chain_monitor.get_and_clear_pending_events().is_empty());
	let spendable_output_events =
		nodes[0].chain_monitor.chain_monitor.get_and_clear_pending_events();
	...
	for event in spendable_output_events {
		if let Event::SpendableOutputs { outputs, channel_id } = event {
			...
			if let SpendableOutputDescriptor::StaticPaymentOutput(_) = &outputs[0] {
				...
				check_spends!(
					spend_tx,
					revoked_claim_transactions
						.get(&spend_tx.input[0].previous_output.txid)
						.unwrap()
				);
			}
		} else {
...
fn do_test_anchors_monitor_fixes_counterparty_payment_script_on_reload(
	confirm_commitment_before_reload: bool,
) {
	...
	let commitment_tx_conf_height = if confirm_commitment_before_reload {
		...
		*nodes[1].chain_monitor.expect_monitor_round_trip_fail.lock().unwrap() = Some(chan_id);
		let commitment_tx_conf_height =
			block_from_scid(mine_transaction(&nodes[1], &commitment_tx));
	...
	if confirm_commitment_before_reload {
		...
		check_added_monitors(&nodes[1], 1);
		let outputs = get_monitor!(nodes[1], chan_id)
			.get_spendable_outputs(&commitment_tx, commitment_tx_conf_height);
		assert_eq!(outputs.len(), 1);
		let spend_tx = nodes[1]
			.keys_manager
			.backing
...
fn do_test_monitor_claims_with_random_signatures(
	anchors: bool, confirm_counterparty_commitment: bool,
) {
	...
	get_monitor!(closing_node, chan_id).broadcast_latest_holder_commitment_txn(
		&closing_node.tx_broadcaster,
		&closing_node.fee_estimator,
		&closing_node.logger,
	);
	...
	check_closed_event!(
		...
		1,
		ClosureReason::CommitmentTxConfirmed,
		[closing_node.node.get_our_node_id()],
		1_000_000
	);
...
fn test_event_replay_causing_monitor_replay() {
	...
	expect_payment_sent(
		...
		None,
		true,
		true, /* expected post-event monitor update*/
	);
	...
	expect_payment_sent(
		...
		None,
		false,
		false, /* expected post-event monitor update*/
	);
}
