...
fn check_blinded_payment_constraints(
	...
	if amt_msat < constraints.htlc_minimum_msat || cltv_expiry > constraints.max_cltv_expiry {
		return Err(());
	}
	Ok(())
}
...
fn check_blinded_forward(
	...
	let amt_to_forward =
		blinded_path::payment::amt_to_forward_msat(inbound_amt_msat, payment_relay).ok_or(())?;
	let outgoing_cltv_value =
		inbound_cltv_expiry.checked_sub(payment_relay.cltv_expiry_delta as u32).ok_or(())?;
	check_blinded_payment_constraints(inbound_amt_msat, outgoing_cltv_value, payment_constraints)?;

	if features.requires_unknown_bits_from(&BlindedHopFeatures::empty()) {
		return Err(());
	}
...
pub(super) fn create_fwd_pending_htlc_info(
	...
	Ok(PendingHTLCInfo {
		routing: PendingHTLCRouting::Forward {
			...
			short_channel_id,
			blinded: intro_node_blinding_point.or(msg.blinding_point).map(|bp| BlindedForward {
...
pub(super) fn create_recv_pending_htlc_info(
	...
	if (!allow_underpay && onion_amt_msat > amt_msat)
		|| (allow_underpay
			&& onion_amt_msat > amt_msat.saturating_add(counterparty_skimmed_fee_msat.unwrap_or(0)))
	{
		return Err(InboundHTLCErr {
			err_code: 19,
	...
	L::Target: Logger,
{
	let (hop, shared_secret, next_packet_details_opt) =
		decode_incoming_update_add_htlc_onion(msg, node_signer, logger, secp_ctx).map_err(|e| {
	...
	Ok(match hop {
		onion_utils::Hop::Forward { next_hop_data, next_hop_hmac, new_packet_bytes } => {
			...
			{
				return Err(InboundHTLCErr { msg: err_msg, err_code: code, err_data: Vec::new() });
			}
		...
		onion_utils::Hop::Receive(received_data) => create_recv_pending_htlc_info(
			received_data,
			shared_secret,
			msg.payment_hash,
			msg.amount_msat,
...
pub(super) fn decode_incoming_update_add_htlc_onion<
	NS: Deref,
	L: Deref,
	...
	L::Target: Logger,
{
	...
	let next_packet_details = match next_hop {
		onion_utils::Hop::Forward {
			...
			let next_packet_pubkey = onion_utils::next_hop_pubkey(
				secp_ctx,
				msg.onion_routing_packet.public_key.unwrap(),
				&shared_secret,
			);
			NextPacketDetails {
				next_packet_pubkey,
				outgoing_scid: short_channel_id,
				outgoing_amt_msat: amt_to_forward,
				outgoing_cltv_value,
			}
		},
		onion_utils::Hop::Forward {
			next_hop_data:
				msgs::InboundOnionPayload::BlindedForward {
					short_channel_id,
					ref payment_relay,
					ref payment_constraints,
					ref features,
					..
				},
			..
		} => {
			let (amt_to_forward, outgoing_cltv_value) =
				match check_blinded_forward(
					msg.amount_msat,
					msg.cltv_expiry,
					&payment_relay,
					&payment_constraints,
		...
		onion_utils::Hop::Forward {
			next_hop_data: msgs::InboundOnionPayload::Receive { .. },
			..
		}
		| onion_utils::Hop::Forward {
...
mod tests {
	...
	fn fail_construct_onion_on_too_big_payloads() {
		...
		let onion_keys =
			super::onion_utils::construct_onion_keys(&secp_ctx, &path, &session_priv).unwrap();
	...
	fn test_peel_payment_onion() {
		...
		let next_onion = match peeled.routing {
			PendingHTLCRouting::Forward { onion_packet, .. } => onion_packet,
			_ => panic!("expected a forwarded onion"),
		};

		let msg2 = make_update_add_msg(amount_msat, cltv_expiry, payment_hash, next_onion);
		let peeled2 =
			peel_payment_onion(&msg2, &charlie, &logger, &secp_ctx, cur_height, true, false)
				.map_err(|e| e.msg)
				.unwrap();
	...
	fn payment_onion_args(
		hop_pk: PublicKey, recipient_pk: PublicKey,
	) -> (
		SecretKey,
		u64,
		u32,
