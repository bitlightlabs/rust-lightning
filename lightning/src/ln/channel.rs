...
impl<SP: Deref> ChannelContext<SP> where SP::Target: SignerProvider  {
		...
		where L::Target: Logger
	{
		...
		println!("debug: build_commitment_transaction");
...
impl<SP: Deref> OutboundV1Channel<SP> where SP::Target: SignerProvider {
	...
	-> Result<Option<msgs::FundingCreated>, (Self, ChannelError)> where L::Target: Logger {
		...
		if self.context.is_colored() {
			println!("rename_rgb_files");
			{
				let x = self.context.color_source.try_lock().unwrap();
			}
			println!("rename_rgb_filesaa");
			self.context.color_source.lock().unwrap().rename_rgb_files(&self.context.channel_id, &temporary_channel_id);
			println!("rename_rgb_filesbb");
		}
...
impl<SP: Deref> Writeable for Channel<SP> where SP::Target: SignerProvider {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
		...
		write_tlv_fields!(writer, {
			...
			(51, self.context.consignment_endpoint, option),
		});
			...
			SP::Target: SignerProvider
{
	fn read<R : io::Read>(reader: &mut R, args: (&'a ES, &'b SP, u32, &'c ChannelTypeFeatures, crate::color_ext::ColorSourceWrapper)) -> Result<Self, DecodeError> {
		...
		read_tlv_fields!(reader, {
			...
			(51, consignment_endpoint, option),
		});
